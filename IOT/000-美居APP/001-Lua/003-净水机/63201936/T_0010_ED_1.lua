...
local KEY_RFID_ERROR = "rfid_error"
local KEY_OVERTIME_ERROR = "overtime_error"
local KEY_TDS_ERROR = "tds_error"
...
local KEY_IN_TDS = "in_tds"
local KEY_OUT_TDS = "out_tds"
...
--根据传入的json解析出的设备状态 byte[] 值修改全局变量值
local function updateGlobalPropertyValueByByte(messageBytes)

    if ((dataType == 0x03 and messageBytes[0] == 0x01) or (dataType == 0x02 and messageBytes[0] == 0x01) or (dataType == 0x02 and messageBytes[0] == 0x02) or (dataType == 0x02 and messageBytes[0] == 0x04)or (dataType == 0x04 and messageBytes[0] == 0x01)) then
        ...
        inTDS = bit.lshift(messageBytes[37], 8) + messageBytes[36]
        outTDS = bit.lshift(messageBytes[39], 8) + messageBytes[38]
    end
end
...
local function assembleJsonByGlobalProperty()
    ...
    --KEY_OVERTIME_ERROR 强制待机故障
    if (bit.band(byte24, 0x10) == 0x10) then
        streams[KEY_OVERTIME_ERROR] = VALUE_ON
    else
        streams[KEY_OVERTIME_ERROR] = VALUE_OFF
    end
    ...
    --KEY_TDS_ERROR TDS通讯故障
    if (bit.band(byte25, 0x20) == 0x20) then
        streams[KEY_TDS_ERROR] = VALUE_ON
    else
        streams[KEY_TDS_ERROR] = VALUE_OFF
    end
    ...
    streams[KEY_IN_TDS] = inTDS
    streams[KEY_OUT_TDS] = outTDS
    streams[KEY_WATER_LITRE] = waterLitre

    --KEY_RFID_ERROR(因布谷不存在缺水和排水状态，所以可以用0x10标识rfid故障)
    if (bit.band(byte14, 0x10) == 0x10) then
        streams[KEY_RFID_ERROR]=VALUE_ON
    else
        streams[KEY_RFID_ERROR]=VALUE_OFF
    end
     --KEY_LACK_WATER

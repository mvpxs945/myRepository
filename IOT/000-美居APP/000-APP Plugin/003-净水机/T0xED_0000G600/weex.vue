    ...
    <div class="wrapper" :style="objClass">
        ...
        <div class="first-up" :style="upClass">
            ...
            <div class="first-up-bottom">
                ...
                <div class="c1">
                    <text class="c1-num">{{filter.CResidue}}%</text>
                    <text class="c1-name">C</text>
                </div>
                <div class="ro">
                    <text class="ro-num">{{filter.ROResidue}}%</text>
                    <text class="ro-name">RO</text>
                </div>
                <div class="ro">
                    <text class="ro-num">{{filter.UFResidue}}%</text>
                    <text class="ro-name">超滤+C</text>
                </div>
            </div>
        </div>
    ...
    created() {
        ...
        nativeService.getDeviceInfo().then((res)=>{
            ...
            nativeService.startCmdProcess("query",cmd.createQueryMessage(),function(res){
                ...
                filterSwitch(parseInt(msg[35],10)+parseInt(msg[36],10)*256,4320,1);
                filterSwitch(parseInt(msg[37],10)+parseInt(msg[38],10)*256,8640,2);
                filterSwitch(parseInt(msg[39],10)+parseInt(msg[40],10)*256,25920,3);
                filterSwitch(parseInt(msg[41],10)+parseInt(msg[42],10)*256,8640,4);
                nativeService.setItem('filterLife',self.filter);
                self.water=parseInt(msg[17],10)+parseInt(msg[18],10)*256;
                if(self.filter.ROResidue<=5||self.filter.CResidue<=5||self.filter.UFResidue<=5||self.filter.PPResidue<=5){
                    if(self.filter.ROResidue==0||self.filter.CResidue==0||self.filter.UFResidue==0||self.filter.PPResidue==0){
        ...
        function filterSwitch(real,max,num){
            switch (num){
                case 1:
                self.filter.PPResidue=parseInt((max-real)/max*100,10)>0?parseInt((max-real)/max*100,10):0
                case 2:
                self.filter.CResidue=parseInt((max-real)/max*100,10)>0?parseInt((max-real)/max*100,10):0
                case 3:
                self.filter.ROResidue=parseInt((max-real)/max*100,10)>0?parseInt((max-real)/max*100,10):0
                case 4:
                self.filter.UFResidue=parseInt((max-real)/max*100,10)>0?parseInt((max-real)/max*100,10):0
            }
        }
    },
    mounted(){

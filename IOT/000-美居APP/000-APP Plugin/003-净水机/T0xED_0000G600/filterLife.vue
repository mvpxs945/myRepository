    ...
    created(){
        ...
        nativeService.getItem("key",function(i){
            switch(i.data){
                ...
                nativeService.getItem('filterLife',function(arr){
                    var realLife=JSON.parse(arr.data)
                    self.life=realLife.UFResidue;
                });
                self.name="UF+C滤芯";
                self.info="UF+C滤芯即超滤和活性炭复合滤芯，主要拦截细菌，胶体，大分子有机物等，使用周期24-26个月（因使用地域、水质、使用量及季节不同，实际使用周期有所不同）";
                break;
            }
        })
    },

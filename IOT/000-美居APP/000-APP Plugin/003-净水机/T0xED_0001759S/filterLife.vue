...
export default {
    ...
    created(){
        ...
        nativeService.getItem("device_sn",function(sn){
            ...
            cmd.dataReport('KH_0xED_0001759S','view','page_view','filterLife',self.deviceSn,{});//埋点
        })
        nativeService.getItem("key",function(i){
            switch(i.data){
                ...
                nativeService.getItem('filterLife',function(arr){
                    ...
                    self.life=realLife[0].text;
                });
                self.name="PPC复合滤芯";
                self.info="PPC复合滤芯主要拦截水中的泥沙、铁锈、悬浮物、余氯及异物等,使用周期6-12个月（因使用地域、水质、使用量及季节不同，实际使用周期有所不同）";
                break;
                case "1":
                self.title="前置PPC滤芯";
                nativeService.getItem('filterLife',function(arr){
                    var realLife=JSON.parse(arr.data)
                    self.life=realLife[1].text;
                });
                self.name="PPC复合滤芯";
                self.info="PPC复合滤芯主要拦截水中的泥沙、铁锈、悬浮物、余氯及异物等,使用周期6-12个月（因使用地域、水质、使用量及季节不同，实际使用周期有所不同）";
                break;

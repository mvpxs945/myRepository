        ...
        nativeService.getItem("key",function(i){
            switch(i.data){
                ...
                nativeService.getItem('status',function(res){
                    ...
                    self.life=parseInt(msg[26]);
                });//读取weex.vue页面存入公共状态的数据
                self.name="PAC复合滤芯";
                self.info="PAC复合滤芯主要拦截水中的泥沙、铁锈、悬浮物、余氯及异物等,使用周期6-12个月（因使用地域、水质、使用量及季节不同，实际使用周期有所不同）";
                break;
                case "1":
                self.title="RO滤芯";
                nativeService.getItem('status',function(res){
                ...
                self.name="活性炭滤芯";
                self.info="CB滤芯即活性炭滤芯，主要吸附异色，异味，余氯，部分有机物（三氯甲烷，四氧化碳），使用周期6-12个月（因使用地域、水质、使用量及季节不同，实际使用周期有所不同）";
                break;
            }
        })
    },

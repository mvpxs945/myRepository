<template>
<div style="paddingRight:30px;backgroundColor:white;">
  <div
    ...
    <slot name="text"></slot>
    <slot name="rightText">
        <div v-if="rightText" :style="{paddingRight:hasArrow?'40px':'0px'}">
           <text class="right-text" :style="rightTextStyle">{{rightText}}</text>
        </div>
    </slot>
    <image :src="iconSrc!==''? iconSrc : arrowIcon" :style="{top:((height-40)/2)+'px'}"
...
<script>
  import util from '../assets/setting/util'
  const icon = require('../assets/setting/icon.base64.js');

    ...
    created(){
        ...
        nativeService.getItem("key",function(i){
            nativeService.getItem("device_sn",function(sn){
                ...
                // cmd.dataReport('KH_0xED_63202029','view','page_view','filterLife',self.deviceSn,{});//埋点
            })
            switch(i.data){
                case "0":
                self.title="PP滤芯";
                self.initStatus(26);
                self.name="PAC/C复合滤芯";
                self.info="PAC/C复合滤芯主要拦截水中的泥沙、铁锈、悬浮物、余氯及异物等,使用周期6-12个月（因使用地域、水质、使用量及季节不同，实际使用周期有所不同）";
                break;
                case "1":
                self.title="C'+C\"滤芯";
                self.initStatus(27);
                ...
                self.initStatus(28);
                self.name="RO滤芯";
                self.info="RO滤芯即反渗透滤芯，主要拦截有机物（三氯甲烷，四氧化碳）、重金属（砷，铅，镉），使用周期24-36个月（因使用地域、水质、使用量及季节不同，实际使用周期有所不同）";
                break;
            }
        })
    },

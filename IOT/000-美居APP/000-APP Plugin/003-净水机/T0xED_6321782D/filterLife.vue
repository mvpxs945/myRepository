    ...
    created(){
        ...
        nativeService.getItem("key",function(i){
            nativeService.getItem("device_sn",function(sn){
                ...
                cmd.dataReport('KH_0xED_6321782D','view','page_view','filterLife',self.deviceSn,{});//埋点
            })
            switch(i.data){
                case "0":
                self.title="RO滤芯";
                ...
                self.initStatus(27);
                self.name="PCB复合滤芯";
                self.info="PCB复合滤芯主要拦截水中的泥沙、铁锈、悬浮物、余氯及异物等,使用周期6-12个月（因使用地域、水质、使用量及季节不同，实际使用周期有所不同）";
                break;
            }
        })
    },

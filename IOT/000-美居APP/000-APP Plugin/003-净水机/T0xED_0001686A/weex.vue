...
export default {
    ...
    methods: {
        goMore(){
            cmd.dataReport('KH_0xED_0001686A','event','go_moreFunction','weex',this.deviceSn,{});//埋点
    ...
    created() {
        ...
        nativeService.getDeviceInfo().then((res)=>{
            ...
            cmd.dataReport('KH_0xED_0001686A','view','page_view','mainPage',res.result.deviceSn,{});//埋点
            //保存当前设备状态到公共状态
            nativeService.startCmdProcess("query",cmd.createQueryMessage(),function(res){
                ...
                self.TDS=parseInt(msg[49],10)+parseInt(msg[48],10)*256;
                self.filterList[0].text=msg[26];
                self.filterList[1].text=msg[27];
                self.filterList[2].text=msg[26];
                if(parseInt(msg[26],10)<=5||parseInt(msg[27],10)<=5||parseInt(msg[26],10)<=5){
                    if(parseInt(msg[26],10)==0||parseInt(msg[27],10)==0||parseInt(msg[26],10)==0){

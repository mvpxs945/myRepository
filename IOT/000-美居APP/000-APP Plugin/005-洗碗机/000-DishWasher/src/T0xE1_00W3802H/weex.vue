    ...
    <div class="wrapper" @viewdisappear="viewdisappear" @viewappear="viewappear">
      ...
      <div class="container">
        <div v-if="curStatus=='standby'" :style="{'padding-bottom':(isipx?ipxBottom+'px':'0px')}" key="standby">
          ...
          <div v-if="isShowFixedLay" class="standby-footer" :style="{'height':(isipx?((screenH*0.3 + 98 + ipxBottom) +'px'):((screenH*0.3 + 98) +'px'))}">
            <div>
            ...
            <div>
              <midea-cell v-if="curMode.hasKeep" title="鲜存" :desc="keepDesc" height="120"  @mideaCellClick="btnClick('WashKeep')">
                <div slot="value">
      ...
      <midea-dialog class="dialog" :show="isKeepShow" cancelText="洗完启动" confirmText="立即启动" title="开启鲜存"
                mainBtnColor="#267AFF" secondBtnColor="#267AFF"
...
export default {
  ...
  methods: {
    ...
    pickKeepTime() {
      ...
      picker.pick({
          ...
          if(dataArr[0] == 0 && dataArr[1] == 0){
            nativeService.toast('鲜存设置时间应大于0小时哦~');
            return;
          }
    ...
    updateUpUI:function(){
      ...
      //03-获取工作剩余时间，预约剩余时间，当前主洗阶段,暂停, 鲜存状态
  ...
  computed:{
    ...
    keepDesc:function(){
      var self = this;
      var desc = '离子杀菌可除异味,防霉防潮(时间可设1~72小时)';
      ...
      if(self.statusObj.keepTime != 0){
        desc='鲜存中,还剩余下: '+self.statusObj.keepTime+'小时';
      }else if(self.isKeepOn){
        desc = '已设定鲜存时间：'+self.statusObj.setKeepTime+'小时';
      }
      return desc;
    },
    isipx: function() {

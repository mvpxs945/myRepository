      ...
      <div class="container">
        <div v-if="curStatus=='standby'" :style="{'padding-bottom':(isipx?ipxBottom+'px':'0px')}" key="standby">
          ...
          <div v-if="isShowFixedLay" class="standby-footer" :style="{'height':(isipx?((screenH*0.3 + 98 + ipxBottom) +'px'):((screenH*0.3 + 98) +'px'))}">
            ...
            <div>
            <midea-cell title="换风" :desc="keepDesc" height="120"  @mideaCellClick="btnClick('')">
                <div slot="value">
...
export default {
  ...
  methods: {
    ...
    flowTabClicked: function(event) {
      ...
      if(self.curMode.mode == 0x05){
        // if(!self.isSetDiy){
          ...
          //01-先判断设备状态
          let time = upStatusObj.programTimeRemaining.value;
          let mainTemp = upStatusObj.mainWashingTemperature.value;
          let washTemp = upStatusObj.piaoWashTemperature.value;
          self.curMode.temp = washTemp;
          if(time == 18){
            item.name = 'DIY1';
            item.washTimes = 1;
            item.mainTemp = mainTemp;
            item.washTemp = washTemp;
          ...
          this.$refs.diyBox.updateCurDIY(item);
        // }
      }
    ...
    pickOrderTime() {
      var self = this;
      if(self.orderTask.taskId){
        var nowStr = dateFormat.formatDateToString(new Date(),'yyyy-MM-dd hh:mm:ss');
        if(nowStr <= self.orderTask.time){
          self.isShowOrderView = true;
          self.goToOrderView();
          return;
        }else{
          httpRequest.makeOrder('delete',[self.orderTask],function(){
            self.isShowOrderView = false;
          });
        }
      }
      ...
      picker.pick({
          ...
          self.orderToCloud(itemsDay,itemsHour,itemsMinute);
        }
      );
    },
    pickAddition() {
    ...
    diyConfirm: function(event){
      ...
      this.isSetDiy = true;
      var self = this;
      if(self.curMode.mode != 0x05) return;
    ...
    btnClick: function(tag) {
      ...
      switch (tag) {
        ...
        case "btn_order_cancel":
          ...
          //     nativeService.toast('当前网络不佳,请稍后重试');
          // });
          httpRequest.makeOrder('delete',[self.orderTask],function(result){
            self.isShowOrderView = false;
            nativeService.toast('已取消当前预约~');
            self.orderTask = {}; 
            self.updateUpUI();
          });
          break;
        case "btn_order_start":
          ...
          // nativeService.alert('下发：'+sendCmd);
          /** 先删除云端预约，再执行指令下发 */
          var cmdArr = self.orderTask.orderCmd.split(',');
          for(let i=0;i<cmdArr.length;i++){
            cmdArr[i]= parseInt('0x'+cmdArr[i]);
          }
          var sendCmd = cmdArr;
          nativeService.showLoading();
          nativeService.startCmdProcess("control",sendCmd,function(suc) {
    ...
    updateUpUI:function(){
      ...
      if (deviceStatus == 0x00 || deviceStatus == 0x01 || isError || deviceStatus == 0x02) {
        if(self.orderTask.taskId && self.isShowOrderView) {
          return;
        }
    ...
    queryOrder(){
      var self = this;
      httpRequest.makeOrder('getAll',[],function(result){
        if(!result || result == null || result.length == 0){
          return;
        }
        self.orderTask = result[0];
      });
    },
    orderToDevice(itemsDay,itemsHour,itemsMinute){
      var self = this;
      var myDate = new Date();//获取系统当前时间,东8区相差8小时
      var currentHour = myDate.getHours();
      var currentMinute = myDate.getMinutes();
      var day = 0;
      var sendAllMinutes = parseInt(itemsHour[self.dataPick.hour])*60+parseInt(itemsMinute[self.dataPick.min]);
      ...
      }else if(itemsDay[self.dataPick.day]=='明天'){
        day = 1;
        sendAllMinutes = day*24*60+parseInt(itemsHour[self.dataPick.hour])*60+parseInt(itemsMinute[self.dataPick.min]);
      }
      var sendHour = parseInt((sendAllMinutes - currentAllMinutes)/60);
      var sendMinute = (sendAllMinutes - currentAllMinutes)%60;
      var additionalMode = self.mideaChecked ? self.curMode.additionalMode:0;
      var sendCmd = cmdFun.cmdOrderWashing(self.curMode.mode, additionalMode, sendHour,sendMinute);
      nativeService.startCmdProcess("control",sendCmd,function(result) {
        var result_arr = result.replace(/\[|]/g, "").split(",");//去掉中括号;
        var analysisObj = cmdFun.analysisCmd(result_arr);
        self.analysisFun(analysisObj);
    ...
    orderToCloud(itemsDay,itemsHour,itemsMinute){
      var self = this;
      var myDate = new Date();//获取系统当前时间,东8区相差8小时
      ...
      myDate.setDate(currentDay);
      myDate.setHours(parseInt(itemsHour[self.dataPick.hour]));
      myDate.setMinutes(parseInt(itemsMinute[self.dataPick.min]));
      var curTimeTemp = dateFormat.formatDateToString(new Date(),'yyyy-MM-dd hh:mm:ss');
      var timeTemp = dateFormat.formatDateToString(myDate,'yyyy-MM-dd hh:mm:ss');
      var orderCmdTemp = cmdFun.createOrderControlMessage(self.curMode.mode,self.curMode.additionalMode);
      orderCmdTemp = dateFormat.formatToHex(orderCmdTemp);
      var taskParams = {
        curTime: curTimeTemp,
        time: timeTemp,
        orderCmd:orderCmdTemp,
        isRepeat:false,
        enable:true,
        mode:self.curMode.mode+'',
        week:''
      };
      httpRequest.makeOrder('add',[taskParams],function(result){
        if(!result || result == null || result.length == 0){
            return;
        }
        self.orderTask = result[0];
    ...
    goToOrderView: function(){
      ...
      // nativeService.alert(taskObj);
      /** 下发预约任务时间setCurTime   当前时间now   预约任务启动时间time */
      var date = dateFormat.formatStringToDate(self.orderTask.time);
      var time = parseInt(date.getTime()/60000);
      var now = parseInt((new Date().getTime()/60000));
      if(time <= now) {
        self.isShowOrderView = false;
        return;
      }
      self.orderHour = parseInt((time-now)/60);
      self.orderHour = self.orderHour < 10 ? '0'+self.orderHour:self.orderHour;
      self.orderMinute = (time-now)%60;
      self.orderMinute = self.orderMinute < 10 ? '0'+self.orderMinute:self.orderMinute;
      let modeValue = parseInt(self.orderTask.mode);

      self.curStatus = "order";
      if(self.isDoorOpen){
        self.statusObj.status='预约中--门开了';
      }else{
        self.statusObj.status='预约中--'+self.cmdStatus.boxUp.program.viewArr[modeValue];
      }
      self.statusObj.ordertime = self.orderHour+":"+self.orderMinute;
      for (let i = 0; i < self.flowTabData.length; i++) {
        if (self.flowTabData[i].mode.mode == modeValue) {
          self.curMode = self.flowTabData[i].mode;
          break;

            ...
            <div><scroller class="guide_list" :style="{'height':+guideH+'px'}">
                <div v-for="(item,i) in list" :key="i"
                    ...
                    <div class="flex-row-sp">
                        <text v-if="item.subTitle" class="item_sec_desc">{{item.subTitle+item.unit}}</text>
...
export default {
    ...
    data () {
        return {
            ...
            list: [
                { name: "DIY程序", subTitle:'DIY1',unit:'',isMargin:true,value:0x00,isShow:true},
                { name: "洗涤次数", subTitle:1,unit:'次',isMargin:true,isShow:false},
                { name: "主洗温度", subTitle:60,unit:'℃',isMargin:false,isShow:false},
                { name: "漂洗温度", subTitle:50,unit:'℃',isMargin:false,isShow:false},
            ],
            dataPick:{
                diyProgram:0,
                diyMainTemp:0,
    ...
    methods: {
        ...
        updateCurDIY:function(item){
            var self = this;
            self.list[0].subTitle = item.name;
            self.list[1].subTitle = item.washTimes;
            self.list[2].subTitle = item.mainTemp;
            self.list[3].subTitle = item.washTemp;
        },
        ...
        selectDiyProgram() {
            ...
            var washTimes =[1,2,3,4,5,6];
            for(let i=0;i<itemsDay.length;i++){
                if(self.list[0].subTitle == itemsDay[i]){
                    self.dataPick.diyProgram = i;
                    break;
                }
            }
        ...
        selectMainTemp() {
            var self = this;
            var itemsDay = [45,50,55,60,65];
             for(let i=0;i<itemsDay.length;i++){
                if(self.list[2].subTitle == itemsDay[i]){
                    self.dataPick.diyMainTemp = i;
                    break;
                }
            }
        ...
        selectWashTemp() {
            var self = this;
            var itemsDay = [50,55,60,65,70];
            for(let i=0;i<itemsDay.length;i++){
                if(self.list[3].subTitle == itemsDay[i]){
                    self.dataPick.diyWashTemp = i;
                    break;
                }
            }

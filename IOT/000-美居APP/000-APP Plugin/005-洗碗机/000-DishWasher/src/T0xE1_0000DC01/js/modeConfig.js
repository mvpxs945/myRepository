export default{
    MODE_AUTO:{
      src: "assets/img/sel_mode/program_all_icon_auto@2x.png",
      text: "智能洗",
      mode: 1,
      dec: "智能分析，自动选择，最佳的清洗方式",
      additionalMode: 0,
      time: '--',
      temp: 68,
      isDiyMode: false,
      stageStep:[
        {name:'主洗',value:1},{name:'漂洗',value:2},{name:'除菌',value:3}]
    },
    MODE_INTENSIVE:{
      src: "assets/img/mode_intensive.png",
      text: "强力",
      mode: 2,
      dec: "重油污餐具清洗",
      additionalMode: 0,
      time: 78,
      temp: 72,
      isDiyMode: false,
      stageStep:[{name:'主洗',value:1},{name:'漂洗',value:2},{name:'除菌',value:3}]
    },
    MODE_NORMAL:{
      src: "assets/img/mode_normal.png",
      text: "标准",
      mode: 3,
      dec: "日常饭后及时清洗",
      additionalMode: 0,
      time: 58,
      temp: 65,
      isDiyMode: false,
      stageStep:[{name:'主洗',value:1},{name:'漂洗',value:2},{name:'除菌',value:3}]
    },
    MODE_ECO:{
      src: "assets/img/mode_eco.png",
      text: "节能",
      mode: 4,
      dec: "更充分的浸泡，省水省电",
      additionalMode: 0,
      showAdditional: true,
      additionalTitle: "叠加少量",
      time: 88,
      temp: 65,
      isDiyMode: false,
      stageStep:[{name:'主洗',value:1},{name:'漂洗',value:2},{name:'除菌',value:3}],
    },
    MODE_GLASS:{
      src: "assets/img/sel_mode/program_all_icon_glass@2x.png",
      text: "玻璃洗",
      mode: 5,
      dec: "精致玻璃器皿专用",
      additionalMode: 0,
      time: 56,
      temp: 60,
      isDiyMode: false,
      stageStep:[{name:'主洗',value:1},{name:'漂洗',value:2}],
    },
    MODE_RAPID:{
      src: "assets/img/mode_rapid.png",
      text: "超快",
      mode: 7,
      dec: "轻污餐具，短时间洗涤",
      additionalMode: 0,
      time: 19,
      temp: 58,
      isDiyMode: false,
      stageStep:[{name:'主洗',value:1},{name:'漂洗',value:2},{name:'除菌',value:3}],
    },
    MODE_SOAK:{
      src: "assets/img/sel_mode/program_all_icon_pre-wash_Soak@2x.png",
      text: "预冲洗",
      mode: 8,
      additionalMode: 0,
      time: 12,
      temp: 36,
      isDiyMode: false,
      stageStep:[{name:'主洗',value:1}],
    },
    MODE_90MIN:{
      src: "assets/img/sel_mode/90min@2x.png",
      text: "90min洗",
      mode: 9,
      additionalMode: 0,
      time: 90,
      temp: 65,
      isDiyMode: false,
      stageStep:[{name:'主洗',value:1},{name:'漂洗',value:2}],
    },
    MODE_CLEAN:{
      src: "assets/img/sel_mode/program_all_icon_self-cleaning@2x.png",
      text: "自清洁",
      mode: 10,
      additionalMode: 0,
      time: 75,
      temp: 65,
      isDiyMode: false,
      stageStep:[{name:'主洗',value:1},{name:'漂洗',value:2}],
    },
    MODE_FRUIT:{
      src: "assets/img/mode_fruit.png",
      text: "果鲜",
      dec: "蔬菜水果部分海鲜清洗",
      mode: 11,
      additionalMode: 0,
      time: 15,
      temp: '--',
      isDiyMode: false,
      stageStep:[{name:'主洗',value:2},{name:'漂洗',value:3}],
    },
    MODE_DIY:{
      src: "assets/img/sel_mode/setting_diy.png",
      text: "自定义洗",
      mode: 12,
      additionalMode: 0,
      time: "-:--",
      temp: "--",
      isDiyMode: true,
      stageStep:[{name:'预洗',value:1}],
      diyProgram:[
        {name:'DIY1',mainTemp:0, washTemp:0,mainTempRange:[60],washTempRange:[50],stageStep:[{name:'预洗',value:1}]},
        {name:'DIY2',mainTemp:0, washTemp:0,mainTempRange:[60],washTempRange:[50],stageStep:[{name:'预洗',value:1}]},
        {name:'DIY3',mainTemp:0, washTemp:0,mainTempRange:[45,50,55,60,65],washTempRange:[50,55,60,65,70],stageStep:[{name:'预洗',value:1},{name:'主洗',value:2},{name:'漂洗',value:3},{name:'干燥',value:4}]},
        {name:'DIY4',mainTemp:0, washTemp:0,mainTempRange:[45,50,55,60,65],washTempRange:[50,55,60,65,70],stageStep:[{name:'预洗',value:1},{name:'主洗',value:2},{name:'漂洗',value:3},{name:'干燥',value:4}]},
        {name:'DIY5',mainTemp:0, washTemp:0,mainTempRange:[45,50,55,60,65],washTempRange:[50,55,60,65,70],stageStep:[{name:'预洗',value:1},{name:'主洗',value:2},{name:'漂洗',value:3},{name:'干燥',value:4}]},
        {name:'DIY6',mainTemp:0, washTemp:0,mainTempRange:[45,50,55,60,65],washTempRange:[50,55,60,65,70],stageStep:[{name:'预洗',value:1},{name:'主洗',value:2},{name:'漂洗',value:3},{name:'干燥',value:4}]},
      ],
      diySelectIndex:0,
    },

    getSupportMode: function(){
      var self = this;
      var modeArr = [
        self.MODE_NORMAL,
        self.MODE_RAPID,
        self.MODE_INTENSIVE,
        self.MODE_FRUIT,
        self.MODE_ECO,
      ]
      return modeArr;
    }
}
/** 布谷的主洗--漂洗--消毒 === 预洗--主洗--漂洗  名字换但是阶段数值没变 */
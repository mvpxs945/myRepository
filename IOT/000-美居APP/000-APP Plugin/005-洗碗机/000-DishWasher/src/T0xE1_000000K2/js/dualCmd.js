...
var statusObj = {
  boxUp:{
    ...
    program:{
      ...
      viewArr:["空档","智能洗","强力洗","即时洗","节能洗","玻璃洗","1小时洗","超快洗","预冲洗","90分钟洗","自清洁","水果洗"]
    },
    functionOptions:{
    ...
    displaySign:{
      name:"显示标志",
      detail:{
          doorSwitch:{name:"门开关",value:0x01,view:{0:"开门",1:"关门"}},
...
export default {
  ...
  // 查询指令   上柜00,下柜01
  createQueryMessage(box) {
    var queryType = box == 'TYPE_BOX_DOWN'? 0x02:0x00; 
    var messageBody = message.createMessageBody(1); 
    message.setByte(messageBody, 0, queryType);
  ...
  //控制启动指令
  createControlMessage(box,mode, additionalMode) {
    var queryType = box == 'TYPE_BOX_DOWN'? 0x02:0x01; 
    ...
    message.setByte(messageBody, 9, queryType);
    var sendcmd = message.createMessage(0xe1, 0x02, messageBody);
    //nativeService.alert(sendcmd);    
    return sendcmd;
  },
   //预约中直接启动指令
   createOrderControlMessage(box,mode, additionalMode) {
    var queryType = box == 'TYPE_BOX_DOWN'? 0x02:0x01; 
    var messageBody = message.createMessageBody(24); 
    message.setByte(messageBody, 0, 0x08);
    message.setByte(messageBody, 1, 0x03);
  ...
  //预约指令
  cmdOrderWashing:function(box,mode,additionalMode,timeH,timeM){
    var queryType = box == 'TYPE_BOX_DOWN'? 0x02:0x01; 
  ...
  //取消预约指令
  cmdCancelOrderWashing:function(box){
      var queryType = box == 'TYPE_BOX_DOWN'? 0x02:0x01; 
      var messageBody = message.createMessageBody(13);
      message.setByte(messageBody,0,0x08);
   ...
   //设定保管时间
   cmdSetSafeKeep:function(box,value){
    var queryType = box == 'TYPE_BOX_DOWN'? 0x02:0x01; 
    var messageBody = message.createMessageBody(12);
  ...
  //设置感应开关
  cmdSetSensor:function(value){
    var messageBody = message.createMessageBody(12);
    message.setByte(messageBody,0,0x87);
    message.setByte(messageBody,1,value);
    var sendMessage = message.createMessage(0xE1,0x02,messageBody);
    return sendMessage;
  },
  ...
  analysisCmd: function(requestCmd) {
    var obj ={};
    if(parseInt(requestCmd[36]) == 0x01){
      obj = statusObj.boxUp;
    }else if(parseInt(requestCmd[36]) == 0x02){
      obj = statusObj.boxDown;
    }
  
  if((parseInt(requestCmd[9])==2 || parseInt(requestCmd[9])==3 || parseInt(requestCmd[9]) ==4) && (parseInt(requestCmd[10])==0 || parseInt(requestCmd[10])==0x02)){
    ...
    obj.sensor.value = parseInt(requestCmd[37]);
   }
    return statusObj;
  },
  ...
  queryStatus(upCallback,downCallback){
    var self = this;
    var cmd = self.createQueryMessage("TYPE_BOX_UP");
    nativeService.startCmdProcess("query",cmd,function(suc) {
        var arr = suc.replace(/\[|]/g, "").split(",");//去掉中括号
        var statusObj = self.analysisCmd(arr);
        if(upCallback != undefined){
          upCallback(statusObj);
        }
      },function(fail) {
        nativeService.toast('当前网络不佳，请稍后重试');
      }
    );
    var downCmd = self.createQueryMessage("TYPE_BOX_DOWN");
    nativeService.startCmdProcess("query",downCmd,function(suc) {
        var arr = suc.replace(/\[|]/g, "").split(",");//去掉中括号
        var statusObj = self.analysisCmd(arr);
        if(downCallback != undefined){
          downCallback(statusObj);
        }
      },
      function(fail) {
        nativeService.toast('当前网络不佳，请稍后重试');

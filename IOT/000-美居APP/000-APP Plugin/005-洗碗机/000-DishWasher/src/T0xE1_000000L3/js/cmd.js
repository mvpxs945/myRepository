...
var lastCmdType = 0x00;
var lastCmdTime = 0x00;
...
var statusObj = {
  boxUp:{
    ...
    program:{
      name:"程序",value:0x00,
      // view:{0:"空档",1:"智能洗",2:"强力洗",3:"即时洗",4:"节能洗",5:"玻璃洗",6:"1小时洗",7:"超快洗",8:"预冲洗",9:"90分钟洗",10:"自清洁洗",11:"水果洗",12： "自定义洗",13:"除菌洗",14:"碗勺洗",15："紫外除菌"},
      viewArr: ["空档", "智能洗", "强力洗", "标准洗", "节能洗", "玻璃洗", "1小时洗", "超快洗", "预冲洗", "90分钟洗", "自清洁", "水果洗", "自定义洗", "除菌洗","碗勺洗","紫外除菌"]
    },
    functionOptions:{
    ...
    fucTag:{
      //此项目前用作显示标志：
      // BIT0( 干燥开关 ): 0-使能打开，1-使能关闭
      // BIT1( UV开关 ): 0-uv关闭，1-使能打开
      // BIT2(净水洗开关)：0-关闭，1-使能打开
      // BIT3(是否DIY洗)：0-非DIY洗  1-DIY洗
      // BIT4(是否烘干中)：0-不烘干  1-烘干中
      name:"功能标志位",
      fucArr:["保管","抽屉选择","干燥","UV","净水洗","烘干"],
      keepFucSwitch:{type:4,name:"保管",value:0x00,view:{0:"取消保管",1:"使能保管",2:"使能并立即启动保管"}},
      drawerFucSwitch:{type:5,name:"抽屉选择",value:0x00,view:{0:"非抽屉机",1:"抽屉机上抽屉",2:"抽屉机下抽屉"}},
      dryStepSwitch:{type:6,name:"干燥",value:0x00,view:{0:"干燥关",1:"干燥开"}},
      uvFucSwitch:{type:7,name:"UV",value:0x00,view:{0:"UV除菌关",1:"UV除菌开"}},
      jsxFucSwitch:{type:8,name:"净水洗",value:0x00,view:{0:"净水洗关",1:"净水洗开"}},
      dryFucSwitch:{type:9,name:"烘干",value:0x00,view:{0:"烘干关",1:"烘干开"}}
    },
    displaySign:{
      name:"显示标志",
      detail:{
...
export default {
  ...
  //控制启动指令
  createControlMessage(mode, additionalMode,setTime) {
    ...
    message.setByte(messageBody, 10, setTime);
    var intdiy = (mode == 5||mode == 8||mode == 11)?0x04:0;
    message.setByte(messageBody, 15, intdiy);
    var sendcmd = message.createMessage(0xe1, 0x02, messageBody);
    //nativeService.alert(sendcmd);    
    return sendcmd;
  },
   //预约中直接启动指令
   createOrderControlMessage(mode, additionalMode,setTime) {
    setTime = setTime == undefined?0:setTime;
    var messageBody = message.createMessageBody(24); 
    message.setByte(messageBody, 0, 0x08);
    message.setByte(messageBody, 1, 0x03);
  ...
  //预约指令
  cmdOrderWashing:function(mode,additionalMode,timeH,timeM,setTime){
    setTime = setTime == undefined?0:setTime;
  ...
  cmdSafeKeep:function(value){
   ...
   //启动关闭功能
   cmd0x81Fuc:function(type,value){
     var messageBody = message.createMessageBody(15);
     message.setByte(messageBody,0,0x81);
    switch(type){
      case 1://保鲜工作时间(暂未使用)
      ...
      break;
      case 5://抽屉选择
      message.setByte(messageBody,5,value);
      break;
      case 6://干燥开关
      message.setByte(messageBody,6,value);
      break;
      case 7://UV灯开
      message.setByte(messageBody,7,value);
      break;
      case 8://净水洗开关
      message.setByte(messageBody,8,value);
      break;
      case 9://烘干打开
      message.setByte(messageBody,9,value);
      break;
    }
    var sendMessage = message.createMessage(0xE1,0x02,messageBody);
    return sendMessage;
  ...
  //设定杀菌时间
  cmdSetKillTime:function(action,time){
    time = time >10 ? 10:time <0 ?0:time;
    var messageBody = message.createMessageBody(15);
    message.setByte(messageBody,0,0x81);
    message.setByte(messageBody,4,0xff);
    message.setByte(messageBody,7,action);
    message.setByte(messageBody,8,time);
    var sendMessage = message.createMessage(0xE1,0x02,messageBody);
    return sendMessage;
  },
   //设置软水盐档位
  ...
  analysisCmd: function(requestCmd) {
    var statusUp =statusObj.boxUp;
    var cmdType = parseInt(requestCmd[9]);
    if(parseInt(requestCmd[10])!=0) {
      return statusUp;
    }
    if(lastCmdType == 0x02 && cmdType == 0x04){
      let curTime = new Date().getTime();
      if((curTime-lastCmdTime) <= 3000){
        return statusUp;
      }
    }
  ...
  if((cmdType ==2 || cmdType ==3 || cmdType ==4)){
    statusUp.workingState.value = parseInt(requestCmd[11]);    
    statusUp.program.value = parseInt(requestCmd[12]);      
    statusUp.functionOptions.value = parseInt(requestCmd[13]); 
    var byte14Str = this.tranformTo2Bit(parseInt(requestCmd[14]));
    statusUp.fucTag.dryStepSwitch.value = parseInt(byte14Str[7-0]); 
    statusUp.fucTag.uvFucSwitch.value = parseInt(byte14Str[7-1]); 
    statusUp.fucTag.jsxFucSwitch.value = parseInt(byte14Str[7-2]); 
    // statusUp.fucTag.diyFucSwitch.value = parseInt(byte14Str[7-3]); 
    statusUp.fucTag.dryFucSwitch.value = parseInt(byte14Str[7-4]); 
    var byte15Str = this.tranformTo2Bit(parseInt(requestCmd[15]));
    //nativeService.alert(byte15Str);
    statusUp.displaySign.detail.doorSwitch.value = parseInt(byte15Str[7-0]); 
    statusUp.displaySign.detail.brightDishAgent.value = parseInt(byte15Str[7-1]); 
    statusUp.displaySign.detail.salt.value = parseInt(byte15Str[7-2]); 
    statusUp.displaySign.detail.startStop.value = parseInt(byte15Str[7-3]); 
    statusUp.displaySign.detail.lockable.value = parseInt(byte15Str[7-4]); 
    statusUp.displaySign.detail.keepFresh.value = parseInt(byte15Str[7-5]); 
    // nativeService.toast(parseInt(byte15Str[7-3]));
    statusUp.displaySign.detail.noWater.value = parseInt(byte15Str[7-6]);  
    statusUp.programTimeRemaining.value = parseInt(requestCmd[16]); 
    statusUp.appointmentHour.value = parseInt(requestCmd[17]); 
    statusUp.appointmentMinute.value = parseInt(requestCmd[18]); 
    statusUp.washingStage.value = parseInt(requestCmd[19]); 
    statusUp.errorCode.value = parseInt(requestCmd[20]); 
    //故障
    if(statusUp.errorCode.value > 0){
      statusUp.isError.value = true;
    }else{
      statusUp.isError.value = false;
    }
    statusUp.realTimeTemperature.value = parseInt(requestCmd[21]); 
    statusUp.waterLevel.value = parseInt(requestCmd[23]); 
    statusUp.setKeepTime.value = parseInt(requestCmd[27]);
    statusUp.keepTime.value = parseInt(requestCmd[28]);
    //statusUp.washingTimes.value = parseInt(requestCmd[29]); 
    statusUp.setAppointmentHour.value = parseInt(requestCmd[29]);
    statusUp.setAppointmentMinute.value = parseInt(requestCmd[30]);
    statusUp.mainWashingTemperature.value = parseInt(requestCmd[31]); 
    statusUp.piaoWashTemperature.value = parseInt(requestCmd[33]);  
    statusUp.fenpeiqiLevel.value = parseInt(requestCmd[34]); 
    statusUp.totalMinutes.value = parseInt(requestCmd[39]); 
   }
   return statusObj;
  },

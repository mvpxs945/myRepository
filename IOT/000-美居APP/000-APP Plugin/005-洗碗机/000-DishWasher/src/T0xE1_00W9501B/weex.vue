      ...
      <div class="container">
        ...
        <div class="standby-status">
          <div class="flex-column-center">
            <div class="tab-area">
              <midea-tab  class="tabFrame" :tabArray="tabData" :cuStyle="tabStyle" @tabClicked="tabClicked"/>
              <image v-if="false" :src="icon.smartHelper" class="smart-helper" @click="btnClick('smartHelper')"/>
            </div>
          </div>
          <div class="status flex-column-center">
              <text :class="[curMode.time >99 ? 'status-time-small':'status-time']">{{curMode.time}}</text>
              <text class="status-time-desc">{{curMode.dec}}</text>
          </div>
          <div class="flex-column-center">
          ...
          <text :class="[curMode.time >99 ? 'status-about':'status-about-small']">约</text>
          <div class="error-bar-lay" v-if="isErrorBoxShow" @click="errorShow">
          ...
          <div>
            <midea-cell title="保管" :desc="keepDesc" height="120"  @mideaCellClick="btnClick('WashKeep')">
              <div slot="value">
                <midea-switch2 ref="isKeepOn" class="power-switch" @change="keepSwith"></midea-switch2>
              </div>
            </midea-cell>
      ...
      <div v-if="curStatus=='working'" class="container" :style="{height:(screenH-128)+'px','padding-bottom':(isipx?ipxBottom+'px':'0px')}" key="working">
        <div class="standby-status flex-column-center">
          ...
          <div class="status-circle flex-column-center">
              ...
              <div ref="workingBg" class="DashBoard-move"></div>
          </div>
      ...
      <div v-if="curStatus=='order'" class="container" :style="{height:(screenH-128)+'px','padding-bottom':(isipx?ipxBottom+'px':'0px')}" key="order">
        <div class="standby-status flex-column-center">
          ...
          <div class="status-circle flex-column-center">
              ...
              <div ref="orderBg" class="DashBoard-move"></div>
          </div>
          <text v-if="false" class="status-work-unit">{{Language.statusUnit}}</text>
          <text class="status-work-ing">{{statusObj.status}}</text>
          <text class="status-work-end">预计{{finishTime}}左右完成洗涤</text>
        </div>
        <div class="down-block flex-row-sp" :style="{'bottom':(isipx?ipxBottom+60+'px':'60px')}">
...
import mideaDualDialog from "./components/errorDualDialog.vue";
...
export default {
  ...
  methods: {
    ...
    tabClicked: function(index) {
      ...
      if(self.curBox == 'TYPE_BOX_UP'){
        self.updateUpUI(self.cmdStatus.boxUp,true);
      }else {
        self.updateUpUI(self.cmdStatus.boxDown,false);
      }
    },
    ...
    errorShow:function(){
      ...
      nativeService.goTo('errorDetail.js',{});
    },
    ...
    keepCmdControl:function(keepValue){
      ...
      var cmd = cmdFun.cmdSafeKeep(self.curBox,keepValue);
    ...
    keepSwith: function(event) {
      var self = this;
      if(self.curBox == 'TYPE_BOX_UP' && self.isUpError){
        setTimeout(function(){
      ...
      if(self.curBox == 'TYPE_BOX_DOWN' && self.isDownError){
        setTimeout(function(){
          Vue.set(self.$refs.isKeepOn,'checked',self.isKeepOn);
        },1000);
        nativeService.toast('故障中,无法操作~'+self.isKeepOn);
        return;
      }
      if(self.curBox == 'TYPE_BOX_UP' && self.isUpDoorOpen){
        setTimeout(function(){
          Vue.set(self.$refs.isKeepOn,'checked',self.isKeepOn);
        },1000);
      ...
      if(self.curBox == 'TYPE_BOX_DOWN' && self.isDownDoorOpen){
        setTimeout(function(){
          Vue.set(self.$refs.isKeepOn,'checked',self.isKeepOn);
        },1000);
        nativeService.toast('门开中,无法操作~');
      ...
      if(self.isKeepOn){
        ...
        self.keepContentList = [
          '是否立即开启保管',
          '还是洗涤完成后自动开启保管?'
        ]
        self.isKeepShow = true;
      }else {
        let keepValue = 0x00;
    ...
    pickKeepTime() {
      ...
      picker.pick({
          ...
          var sendCmd = cmdFun.cmdSetSafeKeep(self.curBox,allHour);
    ...
    pickOrderTime() {
      ...
      picker.pick({
          ...
          var sendCmd = cmdFun.cmdOrderWashing(
            self.curBox,self.curMode.mode,self.curMode.additionalMode,sendHour,sendMinute);
          // nativeService.alert('下行：'+sendCmd);
          nativeService.showLoading();
    ...
    btnClick: function(tag) {
      ...
      switch (tag) {
        ...
        case "btn_order_cancel":
          sendCmd = cmdFun.cmdCancelOrderWashing(self.curBox);
        ...
        case "btn_order_start":
          ...
          sendCmd = cmdFun.createOrderControlMessage(self.curBox,self.curMode.mode,self.curMode.additionalMode);
          // nativeService.alert('下发：'+sendCmd);
          nativeService.showLoading();
          nativeService.startCmdProcess("control",sendCmd,function(suc) {
            nativeService.hideLoading();
        ...
        case "btn_start":
          ...
          sendCmd = cmdFun.createControlMessage(self.curBox,self.curMode.mode,self.curMode.additionalMode);
          // nativeService.alert('下发：'+sendCmd);
          nativeService.showLoading();
          nativeService.startCmdProcess("control",sendCmd,function(suc) {
        ...
        case "btn_work_cancel":
          sendCmd = cmdFun.cmdCancelWash(self.curBox);
          // nativeService.alert('下发：'+sendCmd);
          nativeService.showLoading();
          nativeService.startCmdProcess("control",sendCmd,function(suc) {
            // nativeService.alert('上行：'+result);
            nativeService.hideLoading();
          var arr = suc.replace(/\[|]/g, "").split(",");//去掉中括号
          var statusObj = cmdFun.analysisCmd(arr);
          self.analysisFun(statusObj);
        ...
        case "btn_work_pause":
          ...
          sendCmd = cmdFun.cmdStartPause(self.curBox,pauseRecord);
          // nativeService.alert('下发：'+sendCmd);
          nativeService.showLoading();
          nativeService.startCmdProcess("control",sendCmd,function(suc) {
            nativeService.hideLoading();
    ...
    analysisFun: function(status) {
      ...
      if(boxType == "TYPE_BOX_UP") {
        let upStatusObj = self.cmdStatus.boxUp;
        self.updateUpUI(upStatusObj);
      }else if(boxType == "TYPE_BOX_DOWN"){
        let upStatusObj = self.cmdStatus.boxDown;
        self.updateUpUI(upStatusObj);
      }
      self.updateErrorUI();
    },
    updateUpUI:function(upStatusObj,isBoxUp){
      var self = this;
      //01-先判断设备状态
      ...
      if (deviceStatus == 0x00 || deviceStatus == 0x01 || isError) {
        ...
        self.orderLeftTime = parseInt(appHourUp)*60+parseInt(appMinuteUp);
        for (let i = 0; i < self.flowTabData.length; i++) {
          if (self.flowTabData[i].mode.mode == modeValue) {
            self.curMode = self.flowTabData[i].mode;
            break;
        ...
        if(self.statusObj.curStage <= 0x02 && modeValue == 0x01){ //智能洗 预洗阶段 没有时间显示
          self.statusObj.time = '--';
        }else {
    ...
    updateErrorUI:function(){
      ...
      /** 门状态 */
      self.isUpError = upStatusObj.isError.value;
      self.isUpDoorOpen = (upStatusObj.displaySign.detail.doorSwitch.value == 0x00);
      let isUpLackSalt = (upStatusObj.displaySign.detail.salt.value == 0x01);
      let isUpLackBright = (upStatusObj.displaySign.detail.brightDishAgent.value == 0x01);
    
      let upErrorCode = upStatusObj.errorCode.value;
      var errorStr = '';
      if(isUpLackSalt){
        errorStr += 'ERROR_UP_LACK_SALT,';
      }
      if(isUpLackBright){
        errorStr += 'ERROR_UP_LACK_BRIGHT,';
      }
      if(self.isUpDoorOpen){
        errorStr += 'ERROR_UP_DOOR_OPEN,';
      }
      if(upErrorCode == 1) {
        errorStr += 'ERROR_UP_E1,';
      }else if(upErrorCode == 3) {
        errorStr += 'ERROR_UP_E3,';
      }else if (upErrorCode == 4) {
        errorStr += 'ERROR_UP_E4,';
      }else if(upErrorCode == 0x0c) {
        errorStr += 'ERROR_UP_EC,';
      }else if(upErrorCode == 0x0e){
        errorStr += 'ERROR_UP_EH,';
      }

      /** 先上柜 */
      upStatusObj = self.cmdStatus.boxDown;
      // nativeService.alert(JSON.stringify(self.cmdStatus.boxDown));
      /** 门状态 */
      self.isDownError = upStatusObj.isError.value;
      self.isDownDoorOpen = (upStatusObj.displaySign.detail.doorSwitch.value == 0x00);
      let isDownLackSalt = (upStatusObj.displaySign.detail.salt.value == 0x01);
      let isDownLackBright = (upStatusObj.displaySign.detail.brightDishAgent.value == 0x01);
      // nativeService.alert(self.isDownError+':'+self.isDownDoorOpen);
      let errorCode = upStatusObj.errorCode.value;
      if(isDownLackSalt){
        errorStr += 'ERROR_DOWN_LACK_SALT,';
      }
      if(isDownLackBright){
        errorStr += 'ERROR_DOWN_LACK_BRIGHT,';
      }
      if(self.isDownDoorOpen){
        errorStr += 'ERROR_DOWN_DOOR_OPEN,';
      }
      if (errorCode == 1) {
        errorStr += 'ERROR_DOWN_E1,';
      }else if(errorCode == 3) {
        errorStr += 'ERROR_DOWN_E3,';
      }else if (errorCode == 4) {
        errorStr += 'ERROR_DOWN_E4,';
      }else if(errorCode == 0x0c) {
        errorStr += 'ERROR_DOWN_EC,';
      }else if(errorCode == 0x0e){
        errorStr += 'ERROR_DOWN_EH,';
      }
      if(upErrorCode == 0x04 || errorCode == 0x04){
        self.isUpError = true;
        self.isDownError = true;
      }
      // nativeService.alert(JSON.stringify(data));
      self.errorData = data;
      self.isError = self.isUpError || self.isDownError || self.isUpDoorOpen || self.isDownDoorOpen;
      self.isErrorBoxShow = self.isError || isUpLackSalt || isUpLackBright || isDownLackSalt || isDownLackBright;
      nativeService.setItem('error',errorStr);
    },
    openAnimation:function(workingBg,type){
      var self = this;
      var rotateDeg = 0;
      if(type == 'TYPE_WORK' && self.curStatus == 'working'){
        self.workDeg +=360;
        rotateDeg = self.workDeg;
      }else if(type == 'TYPE_ORDER' && self.curStatus == 'order'){
        self.orderDeg +=360;
        rotateDeg = self.orderDeg;
      }
      animation.transition(workingBg, {
          styles: {
  ...
  watch:{
    curStatus:function(curValue,oldValue){
      var self = this;
      if(curValue == 'working'){
        setTimeout(function(){
          var workBg = self.$refs.workingBg;
          self.openAnimation(workBg,'TYPE_WORK');
          setInterval(function () {
            self.openAnimation(workBg,'TYPE_WORK');
          }, 2500);
        },100);
      }else if(curValue == 'order'){
        setTimeout(function(){
          var orderBg = self.$refs.orderBg;
          self.openAnimation(orderBg,'TYPE_ORDER');
          setInterval(function () {
            self.openAnimation(orderBg,'TYPE_ORDER');
          }, 2500);
        },100)
  ...
  computed:{
    ...
    keepDesc:function(){
      ...
      if(self.statusObj.keepTime != 0){
        desc='保管中,还剩余下: '+self.statusObj.keepTime+' H';
      }else{
    ...
    finishTime: function(){
      var self = this;
      let workTime = 0;
      if(self.curMode.mode == modeFun.MODE_AUTO.mode){ // 15-30分钟预留期
        workTime = 150+15;
      }else{
        workTime = parseInt(self.curMode.time)+15;
      }
      let orderTime = self.orderLeftTime;
      let finishTime =  orderTime+workTime;
      var myDate = new Date(); //获取系统当前时间
      let curDay = myDate.getDay();
      let curHour = myDate.getHours();
      let curMin = myDate.getMinutes();
      let timeStamp = myDate.getTime();
      timeStamp = timeStamp + finishTime*60000;
      myDate.setTime(timeStamp);
      let finishDay = myDate.getDay();
      let finishHour = myDate.getHours();
      let finishMin = myDate.getMinutes();
      let finishMinTotal = finishTime + curHour*60+curMin;
      let str ='';
      finishHour = finishHour<10?'0'+finishHour:finishHour;
      finishMin = finishMin<10?'0'+finishMin:finishMin;
      if(finishMinTotal >= 2*24*60){//最多可预约两天
        str = '后天'+finishHour+':'+finishMin;
      }else if(finishMinTotal >=24*60){
        str = '明天'+finishHour+':'+finishMin;
      }else{
        str = '今天'+finishHour+':'+finishMin;
      }
      return str;
    },
  },

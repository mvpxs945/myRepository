    ...
    <div class="wrapper" @viewdisappear="viewdisappear" @viewappear="viewappear">
      ...
      <div class="container">
        <div v-if="curStatus=='standby'" :style="{'padding-bottom':(isipx?ipxBottom+'px':'0px')}" key="standby">
	      ...
	      <midea-cell title="净水洗" :desc="使用净水进行最后一次漂洗" height="120">
                <div slot="value">
                  <midea-switch2 ref="isPurifierOn" :checked="isPurifierOn" class="power-switch" @change="purifierSwith"></midea-switch2>
                </div>
              </midea-cell>
      ...
      <midea-dialog class="dialog" :show="isPurifierShow" cancelText="无净水机" confirmText="有净水机" title="开启净水洗"
        mainBtnColor="#267AFF" secondBtnColor="#267AFF"
        @cancel="dialogClick('noPurifier')"
        @confirm="dialogClick('withPurifier')"
        :errorContentList="purifierContentList"
      ></midea-dialog>
...
export default {
  ...
  methods: {
    ...
    dialogClick:function(tag){
      ...
      switch(tag){
        ...
        case 'keepNowStart':
          ...
          break;
        case "noPurifier":
          self.isPurifierShow = false;
          let purifierValue1 = 0x00;
          self.purifierCmdControl(purifierValue1);
          break;
        case "withPurifier":
          self.isPurifierShow = false;
          let purifierValue2 = 0x01;
          self.purifierCmdControl(purifierValue2);
          break;
	case 'cancelWash':
    ...
    purifierCmdControl: function(purifierValue) {
      //净水洗功能
      var self = this;

      var cmd = cmdFun.cmdPurifier(purifierValue);
      //nativeService.alert("下行：" + cmd);
      nativeService.showLoading();
      nativeService.startCmdProcess(
        ...
        function(suc) {
          ...
          //下发净水洗开启或关闭命令时，为了避免设备上报错误保管状态
          // nativeService.alert('purifierValue'+purifierValue);
          // if (purifierValue == 0x01) {
          //   statusObj.boxUp.regionalOptions.detail.purifierWash.value = 1;
          // } else {
          //   statusObj.boxUp.regionalOptions.detail.purifierWash.value = 0;
          // }
        ...
        function(fail) {
          ...
          nativeService.toast("当前网络不佳,请稍后重试");
          setTimeout(function() {
            Vue.set(self.$refs.isPurifierOn, "checked", self.isPurifierOn);
          }, 500);
        }
      );
    },
...
purifierSwith: function(event) {
      ...
      self.isPurifierOn = event.value;
      if (self.isPurifierOn) {
        // bigData.dataReport("BTN_KEEP");
        self.purifierContentList = ["洗碗机是否已接通净水器?"];
        self.isPurifierShow = true;
      } else {
        let purifierValue = 0x00;
        self.purifierCmdControl(purifierValue);
      }
    },
    pickKeepTime() {
      var self = this;
	...
	self.isPurifierOn = upStatusObj.regionalOptions.detail.purifierWash.value == 0x01;
        //Vue.set(self.$refs.isKeepOn,'checked',self.isKeepOn);
        ...
        if ((self.statusObj.curStage == 0x00 || self.statusObj.curStage == 0x01 ||self.statusObj.curStage == 0x02) && modeValue == 0x01) { //智能洗 预洗阶段 没有时间显示
          self.statusObj.time = '--';
        }else {

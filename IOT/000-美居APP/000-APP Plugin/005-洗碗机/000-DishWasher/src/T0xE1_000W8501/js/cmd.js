...
var statusObj = {
  boxUp: {
    ...
    regionalOptions:{
      ...
      detail: {
        purifierWash: {name: "净水洗",value: 0x00,view: {0: "关闭净水洗",1: "开启净水洗"}},
      }
    },
    displaySign:{
      name:"显示标志",
...
export default {
  ...
  createControlMessage(mode, additionalMode) {
    ...
    message.setByte(messageBody, 9, 0x01);
  ...
  cmdSafeKeep:function(value){
    ...
    message.setByte(messageBody, 5, 0x01);
    message.setByte(messageBody, 9, 0xff);
    var sendMessage = message.createMessage(0xE1, 0x02, messageBody);
    return sendMessage;

  },
  //启动净水洗
  cmdPurifier: function (value) {
    var messageBody = message.createMessageBody(15);
    message.setByte(messageBody, 0, 0x81);
  ...
  cmdStartDiyWashing: function(washTime, washTemperature, dryTemperature){
    ...
    message.setByte(messageBody, 7, 0x01);
    var sendMessage = message.createMessage(0xE1,0x02,messageBody);
    return sendMessage;
},
  ...
  if((cmdType ==2 || cmdType ==3 || cmdType ==4)){
    ...
    obj.regionalOptions.detail.purifierWash.value = parseInt(byte14Str[7 - 2]);

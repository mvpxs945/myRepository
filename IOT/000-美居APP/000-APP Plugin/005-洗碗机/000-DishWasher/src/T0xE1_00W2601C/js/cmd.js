...
var statusObj = {
  boxUp:{
    ...
    program:{
      ...
      viewArr:["空档","智能洗","强力洗","餐具洗","节能洗","奶瓶洗","玩具洗","超快洗","预冲洗","超快洗","自清洁","水果洗","自定义洗"]
    },
    functionOptions:{
...
export default {
  ...
  * 语音包类型
  **/
  cmdCreateVoiceMsgBody : function(voiceType){
    var self = this;
    var messageBody = [];
    let headArr,md5Arr = [];
    switch(voiceType){
      case 1:  // 语音包1
        headArr = [0xCA, 0x01, 0x01, 0x01];//byte10开始 
        md5Arr = self.stingToArray('87F2E7B0C6CEE0C75954828DAA42C05D');
        messageBody= headArr.concat(md5Arr);
        break;
      case 2:  // 语音包2
        headArr = [0xCA, 0x01, 0x02, 0x01];//byte10开始 
        md5Arr = self.stingToArray('7A2FFC36CAE741079622E998D6DFC64C');//16进制转10进制
        messageBody = headArr.concat(md5Arr);
        break;
      case 3:  // 语音包3
        headArr = [0xCA, 0x01, 0x03, 0x01];//byte10开始 
        md5Arr = self.stingToArray('E80506C21656E8DD4E1FE62D73DF0EE6');//16进制转10进制
        messageBody = headArr.concat(md5Arr);
        break;
      case 4:  // 语音包4
        headArr = [0xCA, 0x01, 0x04, 0x01];//byte10开始 
        md5Arr = self.stingToArray('D97FB1AF94583F405C728ED26CFADA61');//16进制转10进制
        messageBody = headArr.concat(md5Arr);
        break;
      case 5:  // 语音包5
        headArr = [0xCA, 0x01, 0x05, 0x01];//byte10开始 
        md5Arr = self.stingToArray('BED79756D2D8357D37645813F1059F27');//16进制转10进制
        messageBody = headArr.concat(md5Arr);
        break;
      case 6:  // 语音包6
        headArr = [0xCA, 0x01, 0x06, 0x01];//byte10开始 
        md5Arr = self.stingToArray('9C45EB4E57D7128FDE4E5C727219AABC');//16进制转10进制
        messageBody = headArr.concat(md5Arr);
        break;
      case 7:  // 语音包7
        headArr = [0xCA, 0x01, 0x07, 0x01];//byte10开始 
        md5Arr = self.stingToArray('B188F509936B0DD3122FBC9F3C62BF0C');//16进制转10进制
        messageBody = headArr.concat(md5Arr);
        break;
      case 8:  // 语音包8
        headArr = [0xCA, 0x01, 0x08, 0x01];//byte10开始
        md5Arr = self.stingToArray('5CA1FF1F64F795612274EB0C72A9FFDA');//16进制转10进制
        messageBody = headArr.concat(md5Arr);
        break;
    }
    var msg = message.createMessage(0xE1, 0x02, messageBody);
    return msg;
  },
  stingToArray: function(ctrlStr){ //控制命令字符串 转为byte数组 , 1byte两位
    var str_arr = [];
    for(var i=0; i<ctrlStr.length; i++){
      var _str = ctrlStr.substring(i,i+2);
      i++;
  ...
  // 取消语音下载
  cmdCancleDownload : function(){
    var messageBody = message.createMessageBody(20);
    message.setByte(messageBody,0,0xCA);
    message.setByte(messageBody,1,0x02);
    var message = message.createMessage(0xE1,0x02,messageBody);
    return message;
  },
  /**
    * 快速还原语音包请求
    *@return {byteArray} 查询请求命令
  */
  cmdQuikRever:function(mode){
    var messageBody = message.createMessageBody(20);
    message.setByte(messageBody,0,0xCA);
    message.setByte(messageBody,1,0x03);
    var message = message.createMessage(0xE1,0x02,messageBody);
  ...
  /**
    ...
    *@return {byteArray} 查询请求命令
  */
  cmdRequestVoiceStatus:function(){
    var messageBody = message.createMessageBody(20);
    message.setByte(messageBody,0,0xCA);
    var message = message.createMessage(0xE1,0x03,messageBody);
    return message;
  },
};

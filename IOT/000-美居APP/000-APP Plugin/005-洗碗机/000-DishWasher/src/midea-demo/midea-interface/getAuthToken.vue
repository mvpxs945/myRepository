<template>
    <div class="wrapper">
        <midea-header title="getAuthToken" :isImmersion="isImmersion" @leftImgClick="back" :showRightImg="true" rightImg="../assets/img/smart_ic_reline@3x.png" @rightImgClick="reload"></midea-header>
        <midea-button text="执行getAuthToken" @mideaButtonClicked="mideaButtonClicked">
        </midea-button>
        <midea-title-bar title="代码"></midea-title-bar>
        <text class="display-block">
            nativeService.getAuthToken().then(
                (resp) => {
                    this.result = resp
                }
            ).catch((error) => {

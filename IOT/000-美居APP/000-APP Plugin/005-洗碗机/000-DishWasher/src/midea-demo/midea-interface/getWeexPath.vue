<template>
    <div class="wrapper">
        <midea-header title="getWeexPath" :isImmersion="isImmersion" @leftImgClick="back" :showRightImg="true" rightImg="../assets/img/smart_ic_reline@3x.png" @rightImgClick="reload"></midea-header>
        <midea-button text="执行getWeexPath" @mideaButtonClicked="mideaButtonClicked">
        </midea-button>
        <midea-title-bar title="代码"></midea-title-bar>
        <text class="display-block">
            nativeService.getWeexPath().then(
                (resp) => {
                    this.result = resp
                }
            ).catch((error) => {

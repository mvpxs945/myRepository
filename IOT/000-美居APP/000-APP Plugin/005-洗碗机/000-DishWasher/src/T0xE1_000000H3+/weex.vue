<template>
    <div :class="[isPowerOn?'wrapper':'wrapper-powerOff']" @viewdisappear="viewdisappear" @viewappear="viewappear">  
      <div class="content" :style="{'height':screenH+'px'}">
        <midea-header :title=Language.title :isImmersion="true"
          :leftImg="isPowerOn?icon.leftImg:icon.leftImg_powerOff"
          :rightImg="isPowerOn?icon.rightImg:icon.rightImg_powerOff"
          ref="lockState"
          :lockStateImg="isPowerOn?(isLockOn?icon.unlockImg:icon.lockImg):icon.lockImg_powerOff"
          :isLockOn="isLockOn"
          @lockClick="lockChange"
          @headerClick="headerClick"
          :titleColor="isPowerOn?style.header.titleColor:style.header_powerOff.titleColor"
          :fontSize="isPowerOn?style.header.fontSize:style.header_powerOff.fontSize"
          :bgColor="isPowerOn?style.header.bgColor:style.header_powerOff.bgColor"
        ></midea-header>
      <!-- :lockStateImg="isPowerOn?(isLockOn?icon.unlockImg:icon.lockImg):icon.lockImg_powerOff" -->
      <div class="container">
        <div v-if="curStatus=='standby'&&statusObj.curStage!=5" :style="{'padding-bottom':(isipx?ipxBottom+'px':'0px')}" key="standby">
          <div class="standby-status">
            <div class="flex-column-center">
              <div class="tab-area">
                <midea-tab  v-if="false" class="tabFrame" :tabArray="tabData" :isPowerOn="isPowerOn" :isLockOn ="isLockOn" :cuStyle="isPowerOn?tabStyle:tabStyle_powerOff" @tabClicked="tabClicked"/>
              </div>
            </div>
            <div class="status flex-row-center">
              <!-- curMode.mode=1是智能洗 -->
              <div v-if="curMode.mode!=1" class="desc-container">
                <text :class="[isPowerOn?'status-time-desc':'status-time-desc-powerOff']">{{Language.statusDes}}</text>
              </div>             
              <text v-if="curMode.mode!=1" :class="[isPowerOn? (curMode.time >99 ? 'status-time-small':'status-time'):(curMode.time >99 ? 'status-time-small-powerOff':'status-time-powerOff')]">{{curMode.time}}</text>
              <text v-if="curMode.mode==1" :class="[isPowerOn? 'status-analysis':'status-analysis-powerOff']">待分析</text>
              <div v-if="curMode.mode!=1" class="unit-container">
                <text :class="[isPowerOn?'status-unit':'status-unit-powerOff']">{{Language.statusUnit}}</text>  
              </div>
            </div>
            <div class="flex-column-center">
                <text v-if="false" class="status-work">待机中</text>
                <text :class="[isPowerOn?'status-temp':'status-temp-powerOff']">水温: {{curMode.temp}}℃</text>
            </div>
            <div v-if="true" class="box-smartHelper flex-column-center" >
              <image  :src="isPowerOn?icon.smartHelper:icon.smartHelper_powerOff" class="smart-helper" @click="btnClick('smartHelper')"/>
              <text :class="[isPowerOn?'text-samrtHelper':'text-samrtHelper-powerOff']">智能助手</text>
            </div>
            <div class="error-bar-lay" v-if="(isError || isDoorOpen || isLockOn) && isErrorBoxShow" @click="errorShow">
              <error-bar :withCloseBtn="withCloseBtn" :msg="errorDec"></error-bar>
            </div>
          </div>
          <div class="standby-footer" :style="{'height':(isipx?((screenH*0.4 + 98 + ipxBottom) +'px'):((screenH*0.4 + 98) +'px'))}">
            <div>
              <div v-if="true" :class="[isPowerOn?'flow-tab-lay':'flow-tab-lay-powerOff']">
                <midea-flow-tab ref="flowTab" :isPowerOn="isPowerOn" :isLockOn ="isLockOn"
            ...
            <div>
              <midea-cell title="保管" v-if="false" :desc="keepDesc" height="120"  @mideaCellClick="btnClick('WashKeep')">
                <div slot="value">
                  <midea-switch2 ref="isKeepOn" :checked="isKeepOn" :isPowerOn="isPowerOn" :isLockOn="isLockOn" class="power-switch" @change="keepSwith"></midea-switch2>
                </div>
              </midea-cell>
              ...
              <div class="flex-row-center" :style="{'height':(screenH*0.3-112)+'px'}">
                <text class="mode-detail">{{modeDetail}}</text>
	            </div>
            </div>
            <!-- <midea-mask class="mask" :opacity="0.6" v-if="!isPowerOn" @click="layoutClick"></midea-mask> -->
            <!-- <div v-if="!isPowerOn">
            <midea-mask @click="layoutClick"></midea-mask>
            </div>                 -->
            <div class="btn-lay flex-row-center" :style="{'bottom':(isipx?ipxBottom+'px':'0px')}">
              <midea-img-btn class="btn-img border-top" :text="isPowerOn?'关机':'开机'" :img="isPowerOn?icon.powerImg:icon.powerImg_powerOff" @Clicked="btnClick('powerImg')"></midea-img-btn>
              <midea-img-btn class="btn-img border-top" text="预约" :img="isPowerOn?icon.btnOrder:icon.btnOrder_powerOff" @Clicked="btnClick('btn_order')"></midea-img-btn>
              <midea-img-btn class="btn-img border-top" text="启动" :img="isPowerOn?icon.btnStart:icon.btnStart_powerOff" @Clicked="btnClick('btn_start')"></midea-img-btn>
            </div>
          </div>
        </div>
        <div v-if="curStatus=='working'&&statusObj.curStage!=5" class="container" :style="{height:(screenH-128)+'px','padding-bottom':(isipx?ipxBottom+'px':'0px')}" key="working">
          <div class="standby-status flex-column-center">
            <div class="flex-column-center">
              <div class="tab-area">
                <midea-tab v-if="false"  class="tabFrame" :tabArray="tabData" :isLockOn ="isLockOn" :cuStyle="tabStyle" @tabClicked="tabClicked"/>
              </div>
            </div>
            <div class="status-circle flex-column-center">
                <text v-if="statusObj.time !='--'" :class="[statusObj.time > 99?'status-work-time-small':'status-work-time']">{{statusObj.time}}</text>
                <text v-if="statusObj.time !='--'" class="status-work-time-desc">{{Language.timeLeft}}</text>
                <!-- 智能洗无时间阶段显示分析中 -->
                <text v-if="statusObj.time =='--'" class="calculating">分析中</text>
                <text v-if="statusObj.time =='--'" :class="[twinkling?'Ellipsis':'Ellipsis-disapper']">...</text>
                <!-- <text v-if="statusObj.time == 0" :class="[statusObj.time > 99?'status-work-time-small':'status-work-time']">{{statusObj.time}}</text> -->
                
            </div>
            <text v-if="statusObj.time!='--'" :class="[statusObj.time > 99?'status-work-unit-small':'status-work-unit']">{{Language.statusUnit}}</text>
            ...
            <div v-show="!statusObj.isPause" class="status-stage flex-row-center">
              <span class="flex-row-center" v-for="(item,index) in curMode.stageStep" :key="index">
                <text  :class="[statusObj.curStage == item.value?'stage-step':'stage-step-normal','text-white']" >{{item.name}}</text>
        ...
        <div v-if="curStatus=='order'&&statusObj.curStage!=5" class="container" :style="{height:(screenH-128)+'px','padding-bottom':(isipx?ipxBottom+'px':'0px')}" key="order">
          <div class="standby-status flex-column-center">
            <div class="flex-column-center">
              <div class="tab-area">
          <midea-tab v-if="false"  class="tabFrame" :tabArray="tabData" :isLockOn ="isLockOn" :cuStyle="tabStyle" @tabClicked="tabClicked"/>
            ...
            <div class="status-circle flex-column-center">
                <text class="status-app-day">{{statusObj.orderDay}}</text>
                <text class="status-app-time">{{statusObj.ordertime}}</text>
                <text class="status-app-time-desc">启动</text>
            </div>
        ...
        <div v-if="statusObj.curStage==5" class="container" :style="{height:(screenH-128)+'px','padding-bottom':(isipx?ipxBottom+'px':'0px')}" key="order">
            <div class="flex-column-center">
              <div class="tab-area">
                <midea-tab  v-if="true" class="tabFrame" :tabArray="tabData" :isLockOn ="isLockOn" :cuStyle="isPowerOn?tabStyle:tabStyle_powerOff" @tabClicked="tabClicked"/>
                <image v-if="true" :src="isPowerOn?icon.powerImg:icon.powerImg_powerOff" class="powerImg" @click="btnClick('powerImg')"/>
              </div>
            </div>  
            <div class="status flex-column-center">
              <!-- curMode.mode=1是智能洗 -->
              <image class="washOver" :src="icon.btnFinish"/>
              <div class="flex-row-center">
                <text class="text-washOver">{{curMode.text}}</text> 
                <text class="text-washOver">完成</text>   
              </div>       
            </div>        
        </div>
      </div>
      ...
      <midea-dialog class="dialog" :show="isKeepShow" cancelText="洗完启动" confirmText="立即启动" title="开启保管"
                mainBtnColor="#267AFF" secondBtnColor="#267AFF"
                @cancel="dialogClick('keepAfterStart')"
                @confirm="dialogClick('keepNowStart')"
                :errorContentList="keepContentList"></midea-dialog>
      <!-- <midea-dialog class="dialog" :show="isPowerShow" cancelText="否" confirmText="是"
                mainBtnColor="#267AFF" secondBtnColor="#267AFF"
                @cancel="dialogClick('powerOn')"
                @confirm="dialogClick('powerOff')"
                :errorContentList="powerContentList"></midea-dialog> -->
...
import mideaMask from './components/mask.vue'
...
import modeFunV2 from "./js/modeConfigVB.js";
...
export default {
  ...
  data() {
    return {
      ...
      ipxBottom: 108,
      Language: Language,
      isPowerOn : false,//false为关机,true为开机
      timer:undefined,
      twinkling:true,
      curStatus: "standby",
      cmdStatus: {},
      ...
      icon: {
        ...
        leftImg_powerOff: "./assets/img/public_ic_gray@3x.png",
        rightImg: "./assets/img/setting.png",
        rightImg_powerOff: "./assets/img/setting_gray@2x.png",
        lockImg:"./assets/img/btn_lock@3x.png",
        lockImg_powerOff: "./assets/img/btn_lock_gray@3x.png",
        ...
        btnOrder: "./assets/img/btn_order@3x.png",
        btnOrder_powerOff: "./assets/img/btn_order_gray@3x.png",
        btnStart: "./assets/img/btn_start@3x.png",
        btnStart_powerOff: "./assets/img/btn_start_gray@3x.png",
        btnFinish:"./assets/img/btn_finish@3x.png",
        smartHelper:"./assets/img/ai_assistant.png",
        smartHelper_powerOff:"./assets/img/ai_assistant_gray@2x.png",
        powerImg:"./assets/img/power_blue@3x.png",
        powerImg_powerOff:"./assets/img/power_gray@3x.png",
      ...
      tabStyle_powerOff: {
        ...
        tabselectedBg: {
          ...
          borderColor: "#717172",
          tabtxtselectedColor: "#e4e4e8"
        },
        tabUnselectedBg: {
          backgroundColor: "#e6e5e9",
          borderColor: "#717172",
          tabtxtUnselectedColor: "#717172"
        }
      },
      tabData: [
  ...
  methods: {
    ...
    lockChange: function(event) {
      ...
      if(self.isPowerOn){
        if(self.isError){
          setTimeout(function(){
            Vue.set(self.$refs.lockState,'isLockOn',self.isLockOn);
          },1000);
        ...
        self.isLockOn = event.value;
        if(self.isLockOn){
          var cmd;
          bigData.dataReport('BTN_LOCK');
          cmd= cmdFun.cmdLock();
        }else{
          cmd = cmdFun.cmdNotLock();
        }  
        ...
        nativeService.startCmdProcess("control",cmd,function(suc) {
          ...
          // setTimeout(function(){
          //   Vue.set(self.$refs.isLockOn,'lockState',self.isLockOn);
          // },500);
        });  
      } 
    },    
    ...
    errorShow:function(){
      ...
      if (self.isDoorOpen || self.isLockOn) {
        return;
      }
    },
    ...
    dialogClick:function(tag){
      ...
      switch(tag){
        ...
        case 'keepNowStart':
          ...
          break;
        // case 'powerOn':
        //   self.isPowerShow = false;
        //   break;           
        // case 'powerOff':  
        //   self.isPowerShow = false;       
        //   let powerOn1 = false;
        //   self.cmdPowerChange(powerOn1);
        //   break;          
    ...
    cmdPowerChange:function(isPowerOn){
      var self = this;
      var sendCmd;
      if(isPowerOn){
        sendCmd = cmdFun.cmdPowerOn();
      }else{
        sendCmd = cmdFun.cmdShutDown();
      }
      nativeService.showLoading();
    ...
    setDiyProgram() {
      ...
      this.isShowFixedLay = false;
    },
    diyConfirm: function(event){
      this.isDiyBoxShow = false;
      this.isShowFixedLay = true;
      var self = this;
    ...
    btnClick: function(tag) {
      var self = this;
      if(self.isPowerOn || tag == "powerImg"){         
        ...
        if(tag == "powerImg"){
          let PowerOn = !self.isPowerOn;
          self.cmdPowerChange(PowerOn);  
          return;
          // if(self.isPowerOn){
          //   self.powerContentList = [
          //     '是否确认关闭当前运行模式？'
          //   ]
          //   self.isPowerShow = true;       
          // }else{
          //   let PowerOn2 = !self.isPowerOn;
          //   self.cmdPowerChange(PowerOn2);                 
          // }          
        }
        if(self.isLockOn){
          nativeService.toast('设备处于童锁状态，请解锁后操作');
    ...
    updateUpUI:function(){    
      ...
      if (deviceStatus == 0x00 || deviceStatus == 0x01 || isError) {
        ...
        self.isLockOn = (upStatusObj.displaySign.detail.lockable.value == 0x01);
       // Vue.set(self.$refs.isKeepOn,'checked',self.isKeepOn);
        ...
        // var leftHourUp = upStatusObj.appointmentHour.value <= 9 ? '0'+upStatusObj.appointmentHour.value:''+upStatusObj.appointmentHour.value;
        // var leftMinuteUp = upStatusObj.appointmentMinute.value <= 9? '0'+upStatusObj.appointmentMinute.value:''+upStatusObj.appointmentMinute.value;
        var leftHourUp = upStatusObj.appointmentHour.value;
        var leftMinuteUp = upStatusObj.appointmentMinute.value;
        var myDate = new Date(); //获取系统当前时间
        var currentHour = myDate.getHours();
        var currentMinute = myDate.getMinutes();
        var AllMinutes = (currentHour+leftHourUp) * 60 + currentMinute + leftMinuteUp;
        var orderHour,orderMinute;
        if(AllMinutes-24*60<0){
          self.statusObj.orderDay = "今天";
          ...
          self.statusObj.orderDay = "明天";
          orderHour = parseInt((AllMinutes-24*60) / 60)<= 9 ? '0'+parseInt((AllMinutes-24*60) / 60) : parseInt((AllMinutes-24*60) / 60);
          orderMinute = parseInt((AllMinutes-24*60) % 60)<= 9 ? '0'+parseInt((AllMinutes-24*60) % 60) : parseInt((AllMinutes-24*60) % 60);
        }
        self.statusObj.ordertime = orderHour+":"+orderMinute;
        for (let i = 0; i < self.flowTabData.length; i++) {
          if (self.flowTabData[i].mode.mode == modeValue) {
            self.curMode = self.flowTabData[i].mode;
            break;
          ...
          if(self.statusObj.isPause){
            ...
            self.timer =setInterval(function(){
              if(self.twinkling){
                ...
                // nativeService.alert(self.$refs.twinkling.style.opasity+'1');
              }else{
                self.twinkling=true;
              }
            },1000);
      ...
      /** 门状态 */
      if(self.isLockOn){
        self.withCloseBtn = false;
      }
      if(isError || doorOpen){
        self.withCloseBtn = true;
      }
      self.isErrorBoxShow = isError || doorOpen || self.isLockOn;
      self.isError = isError;
      self.isDoorOpen = doorOpen;
      if(self.isLockOn){
        self.errorDec = "设备处于童锁状态，请解锁后操作";     
      }else
      if (doorOpen) {
    ...
    queryStatus: function() {
      ...
      cmdFun.queryStatus(function(statusObj){
      	if (statusObj.boxUp.isH3VB.value == 0x00) {
          self.isH3VB = 1;
        } else if (statusObj.boxUp.isH3VB.value == 0x01) {
          self.isH3VB = 2;
        }
        // self.isH3VB = (statusObj.boxUp.isH3VB.value == 0x01);
        self.initData();
        self.analysisFun(statusObj);
      });
    },
...
.wrapper-powerOff{
  ...
  background-color: -moz-linear-gradient(top, #e8e8eb 0%, #d9d8de 100%);
  background-color: -webkit-linear-gradient(top, #e8e8eb 0%, #d9d8de 100%);
  background-color: linear-gradient(to bottom, #e8e8eb 0%, #d9d8de 100%);
  background-color: -o-linear-gradient(top,#e8e8eb, #d9d8de);
  color: white;  
}

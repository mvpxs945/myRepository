<template>
    <div class="wrapper">
        <div class="g-cover" ref="midea-cover" v-if="show" @touchend="hideAction" :style="coverStyle"></div>
        <div ref="midea-popover" class="g-popover" v-if="show && buttons" :style="contentStyle">
            <div class="u-popover-arrow" :style="arrowStyle"></div>
            <div class="u-popover-inner">
                <div :class="['i-btn',i === buttons.length-1 ? 'i-btn-noborder' : '']" v-for="(item,i) in buttons" :key="i" @click="mideaButtonClicked(i,item.key)">
    ...
    methods: {
        mideaPopoverShow() {
            ...
            if (this.hasAnimation) {
                setTimeout(() => this.mideaPopoverAnimationShow(), 40)
            } else {
        ...
        * smooth in
        **/
        mideaPopoverAnimationShow() {
            const popoverEl = this.$refs['midea-popover'];
            const coverEl = this.$refs['midea-cover'];
        ...
        mideaButtonClicked(index, key) {
            if (!!this.animationLock) {
                return;
            }
            this.$emit('mideaPopoverButtonClicked', { key, index });
            this.hideAction();
        },
        /**
        * 隐藏操作

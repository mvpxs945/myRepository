      ...
      <div class="container">
        <div v-if="curStatus=='standby'" :style="{'padding-bottom':(isipx?ipxBottom+'px':'0px')}" key="standby">
          ...
          <div v-if="isShowFixedLay" class="standby-footer" :style="{'height':(isipx?((screenH*0.3 + 98 + ipxBottom) +'px'):((screenH*0.3 + 98) +'px'))}">
            ...
            <div>
              <midea-cell title="UV保管" :desc="keepDesc" height="120"  @mideaCellClick="btnClick('WashKeep')">
                <div slot="value">
...
export default {
  ...
  methods: {
    ...
    keepCmdControl:function(keepValue){
      ...
      nativeService.startCmdProcess("control",cmd,function(suc) {
        ...
        if(keepValue == 0x02){ // 因电控问题无法解决，因此如果立即启动，暂时不刷新处理电控
          self.statusObj.setKeepTime = parseInt(arr[27]);
          self.statusObj.keepTime =parseInt(arr[28]);
          self.isKeepOn = true;
        }else{
  ...
  computed:{
    ...
    keepDesc:function(){
      var self = this;
      var desc = '定期除湿除菌,防霉防返潮(时间可设1~72小时)';

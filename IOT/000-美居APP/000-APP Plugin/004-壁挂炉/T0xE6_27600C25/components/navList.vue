...
<template>
  <div>
      <div :class="[type=='spaceBetween'?'share-wrapper-sb':'share-wrapper']" v-for="row in dList">
             <div  :class="[type=='spaceBetween'?'share-inner-wrapper-sb':'share-inner-wrapper']" v-for="(item,index) in row.items" @click="itemClicked(item.index)" @longpress="itemLongpress(index)" :style="{height:height+'px'}">
